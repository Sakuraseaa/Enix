...
typedef struct intr_stack intr_stack_t;
/***********  线程栈thread_stack  ***********
...
struct task_struct
{
   ...
   /* general_tag是用于线程就绪队列中的结点 */
   struct list_elem general_tag;

   /* all_list_tag是用于线程队列thread_all_list中的结点 */
   ...
   uint32_t parent_pid; // 父进程的pid

   int8_t exit_status; // 进程结束时自己调用exit传入的参数, 进程结束后的返回值

   uint32_t stack_magic; // 用这串数字做栈的边界标记,用于检测栈的溢出
};

#define TASK_NAME_LEN 16
typedef struct task_struct task_status_t;
...
task_status_t *pid2thread(int32_t pid);
