...
struct gate_desc
{
   uint16_t func_offset_low_word; // 中断处理程序在目标代码断内的偏移量的低16位
   uint16_t selector;             // 中断处理程序目标代码段描述符选择子
   uint8_t dcount;                // 此项为双字计数字段，是门描述符中的第4字节。此项固定值，不用考虑
   uint8_t attribute;             // TYPE(D110,类型标记是那种描述符什么), S(标记是系统段还是数据段),
   // DPL, P(段是否在内存)
   uint16_t func_offset_high_word; // 中断处理程序在目标代码断内的偏移量的高16位
};
...
static void pic_init(void)
{
   ...
   outb(PIC_S_DATA, 0x28); // ICW2: 设置起始中断向量号为0x28,也就是IR[8-15] 为 0x28 ~ 0x2F.
   outb(PIC_S_DATA, 0x02); // ICW3: 设置从片连接到主片的IR2引脚
   outb(PIC_S_DATA, 0x01); // ICW4: 8086模式, 设置需要手动关闭中断
...
/**
 * @description: 初始化中断描述符
 * @param {gate_desc} *p_gdesc 中断门描述符结构体
 * @param {uint8_t} attr    TYPE, P, S ,DPL 各种各样的位
 * @param {intr_handler} function 中断处理函数地址
 */
...
void idt_init()
{
   ...
   /* 加载idt */
   uint64_t idt_operand = ((sizeof(idt) - 0) | ((uint64_t)(uint32_t)idt << 16));
   asm volatile("lidt %0"
                :
                : "m"(idt_operand));
