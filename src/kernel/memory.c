...
// 第....个页目录项       -->       0x1x_x000处的页表       -->     未分配
// 第....个页目录项       -->       0x1x_x000处的页表       -->     未分配
// 第1023个页目录项       -->       0x10_0000处的页表       -->     指向的是存放页目录表的4k物理页
//
...
//   系统      0xFFC0_0000 ~ 0xFFFF_EFFF  4GB - 4MB ~   4GB - 4KB       -->     指向第一个页表的物理页                                4092KB                     由于页表还没有分配，所以不知道对应关系
...
typedef struct pool
{
   ...
   lock_t mutex;            // 内存池是共享变量，申请内存时候要保证互斥
} pool_t;
...
static void mem_pool_init(uint32_t all_mem)
{
   ...
   uint32_t used_mem = page_table_size + 0x100000; // 2MB
   ...
   kernel_pool.pool_size = kernel_free_pages * PG_SIZE; // 单位:字节
   // 初始化内核内存池
   kernel_pool.pool_bitmap.btmp_bytes_len = kbm_length;    // 1位-4KB
   ...
   lock_init(&user_pool.mutex);
   lock_init(&kernel_pool.mutex);
   // 内核虚拟内存初始化
   // 内核虚拟地址位图按照物理内存大小初始化
...
//          3.1 首先在页目录表中查询256(字节)偏移对应的页表的物理地址，假设页表物理地址是s
//          3.2 然后在页表物理地址中将偏移4字节对应的页表项，填充物理地址(s + 4)设置为0xD0AF_C000
//      但是存在问题：
//          1. 页目录项可能不存在，此时需要新建一个页表. 来填充页目录项中的页表地址
...
{
   // 先访问页目录本身，然后访问页目录项偏移，得到PTE的地址
   // 要想访问页表，那么就得让cpu以为访问到物理页地址的时候，其实访问的是页表物理地址
   // 然后通过 pte * 4, 访问目标页目录项
...
{
   lock_acquire(&kernel_pool.mutex);
   void *vaddr = malloc_page(PF_KERNEL, pg_cnt);
   if (vaddr != NULL)
      memset(vaddr, 0, pg_cnt * PG_SIZE);
   lock_release(&kernel_pool.mutex);
   return vaddr;
}
...
{
   // 未来可能会有多个用户进程，因此需要上锁
   lock_acquire(&user_pool.mutex);
   void *vaddr = malloc_page(PF_USER, pg_cnt);
   // if (vaddr != NULL)
   memset(vaddr, 0, pg_cnt * PG_SIZE);
   lock_release(&user_pool.mutex);
   return vaddr;
}
...
void block_desc_init(mem_block_desc_t *desc_array)
{
   ...
   // 初始化每种大小的内存块的收货窗口
   for (desc_idx = 0; desc_idx < DESC_CNT; desc_idx++, block_size *= 2)
   {
      // 该售货窗口中售出的内存块的大小
...
void *sys_malloc(uint32_t size)
{
   ...
   arena_t *a;
   mem_block_t *b;
   // 下面要动共享数据(进入临界区)了, 所以提前上锁
   lock_acquire(&mem_pool->mutex);

   if (size > 1024)
   {
      // 超过最大1024字节的mem_block_desc, 直接分配整个页
      uint32_t page_cnt = DIV_ROUND_UP(size + sizeof(arena_t), PG_SIZE) + 1;
